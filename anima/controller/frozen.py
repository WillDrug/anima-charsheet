...
class FrozenWeapon:
    def __init__(self, weapon: Weapon, ability: str = None, def_ability: str = None, formula: str = None):
        ...
        proxied = ['get_bonus_damage', 'get_base_damage', 'get_AP', 'get_speed', 'get_area',
                   'get_fortitude', 'get_breakage', 'get_shielded', 'get_defense', 'get_block',
                   'get_dodge', 'get_initiative']
    ...
    def get_attack(self, limit=None):
        return self.atk if limit is None else max(self.atk, limit)

    def get_defense(self, limit=None):
        return self.defense if limit is None else max(self.defense, limit)
    ...
    def __str__(self):
        return f'<FrozenWeapon(attack {self.get_attack()}, damage {self.get_damage()})>'
...
class Attack:
    ...
    def __init__(self, weapon: FrozenWeapon, location: Location = None, limit=None):
        self.weapon = weapon
    ...
    def get_attack(self):  # calculated and done attack value
        return self.weapon.get_attack(limit=self.limit)

    def get_damage(self):
        return self.weapon.damage_formula(location=self.location)

    def __str__(self):
        return f'<Attack(Weapon: {self.weapon._weapon}, Attack: {self.get_attack()}, ' \
               f'Damage {self.get_damage()})>'

class Defense:
    def __init__(self, weapon: FrozenWeapon, limit=None):
        self.weapon = weapon
        self.limit = limit

    def get_defense(self):
        self.weapon.get_defense(limit=self.limit)